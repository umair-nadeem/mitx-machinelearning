...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    mul_res = np.matmul(theta, feature_vector)
    add_res = mul_res + theta_0
    res = add_res * label
    if res >= 1:
        return 0
    else:
...
def average_perceptron(feature_matrix, labels, T):
    ...
    """
    ftheta = np.zeros(feature_matrix.shape[1])
    ftheta_0 = 0
    theta = np.zeros(feature_matrix.shape[1])
    theta_0 = 0
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
           theta, theta_0 = perceptron_single_step_update(feature_matrix[i], labels[i], theta, theta_0)
           ftheta = ftheta + theta
           ftheta_0 = ftheta_0 + theta_0
    
    total_iters = (T)*(feature_matrix.shape[0])
    return ftheta/(total_iters), ftheta_0/total_iters
